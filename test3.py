...
latest_result = None
mode = 'tag'  # Change to 'gesture' if you want hand tracking active immediately on startup
...
with vision.GestureRecognizer.create_from_options(options) as recognizer:
    try:
        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    match event.key:
                        case pygame.K_t: threading.Thread(target=drone.takeoff, daemon=True).start()
                        case pygame.K_l: threading.Thread(target=drone.land, daemon=True).start()
                        case pygame.K_e: threading.Thread(target=drone.emergency, daemon=True).start()
            ...
            if frame is not None:
                ...
                # --- CRITICAL FIX: Reset gesture tracking data state at loop start ---
                gesture_name = 'None'  
                fingers = 0

                # Refresh battery tracker metric text
                ...
                if not keyboard_active:
                    ...
                    if mode == 'gesture':
                        match gesture_name:
                            case 'Closed_Fist': automated_rc = [0, 0, 0, 0]
                            case 'Pointing Down': automated_rc[2] = -25
                            case 'Pointing Up': automated_rc[2] = 25
                            case 'Pointing Left': automated_rc[0] = -25
                            case 'Pointing Right': automated_rc[0] = 25
                            case 'Thumbs_Up':
                                mode = 'tag'
                                print("TRANSITION: [tag mode active]")
                        ...
                        match primary:
                            ...
                            case 4: 
                                automated_rc[3] = 0
                                mode = 'gesture'
                                print("TRANSITION: [gesture mode active]")
                    
                    rc_control = automated_rc

                # Send execution commands when structural elements differ from preceding cycle values
                if rc_control != last_rc_control:
                    drone.send_rc_control(*rc_control)
                    last_rc_control = rc_control.copy()

                if frame is not None:
                    # Transform standard matrix properties into compatible Pygame graphics formats
        ...
        # Emergency safety resource exit systems
        drone.land()
        drone.streamoff()
        pygame.quit()
        drone.end()