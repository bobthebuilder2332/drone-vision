...
#drone = Tello()
# drone.connect()  # Commented out to prevent script from crashing without drone connection

battery = "Battery: 100%" # Fallback string for local testing
print(f"{battery:=^40}")

# --- CHANGED ONLY THE CAMERA FEED INITIALIZATION ---
# Replaced drone stream setup with MacBook built-in webcam initialization
...
with vision.GestureRecognizer.create_from_options(options) as recognizer:
    try:
          ...
          while running: 
                ...
                # --- CHANGED ONLY THE FRAME CAPTURE SOURCE ---
                # Replaced drone frame read with webcam capture read
                ret, frame = cap.read()
                if not ret:
                      continue
                
                # Optional: Uncomment the line below if you want the webcam to look like a mirror
                      ...
                      if not keyboard_active:
                          ...
                          if mode == 'tag' and len(detected_ids) > 0:
                              ...
                              if primary_tag == 0:
                                  ...
                                  mode = 'gesture' # Switch back to gesture mode
                ...
                if rc_control != last_rc_control:
                      try:
                          ...
                          print(f"RC Control: {rc_control} | Mode: {mode} | Gesture: {gesture_name} | Detected Tags: {detected_ids}")
                      except:
                          pass
